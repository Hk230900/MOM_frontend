...
export default function MeetingDetailPage({ params }) {
  ...
  async function loadMeetingDetails() {
    try {
      ...
      // Check visibility of this meeting based on organization
      const loggedInUsername = api.getUsername();
      const currentUser = usersData.find(u => u.emailid === loggedInUsername);
      const isMasterAdmin = loggedInUsername === 'harshadabk2309@gmail.com';
      const userOrg = currentUser?.organization || 'iSyra';

      if (meetingData.project?.organization && meetingData.project.organization !== userOrg && !isMasterAdmin) {
  ...
  const handleToggleActionComplete = async (itemId) => {
    ...
    const formattedActionItems = updatedActions.map(({ id, text, assignee_id, assignee_name, completed, assignees }) => ({
      id,
      text,
      assignee_id,
      assignee_name,
  ...
  const copyToClipboard = () => {
    ...
    const actionsMarkdown = actionItems.length > 0
      ? actionItems.map(item => {
          ...
          return `- [${item.completed ? "x" : " "}] ${item.text} (Assignees: ${assigneesStr})`;
        }).join("\n")
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center space-x-2 mt-1">
                        <span className="text-[10px] font-semibold text-indigo-400 bg-indigo-500/10 px-2 py-0.5 rounded">
                          Assignees: {item.assignees && item.assignees.length > 0
                            ? item.assignees.map(a => a.name).join(", ")
                            : item.assignee_name || "Unassigned"}
                        </span>
                        {item.completed && (
